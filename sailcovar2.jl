...
@constraint(m, h[1] == 10)
@constraint(m, h[5] >= 10)
@constraint(m, loop[i in 1:4], h[i]+x[i]+y[i]==d[i]+h[i+1])  
@constraint(m, x[1]+y[1]-50== Kp[1]+Zp[1])
@constraint(m, secondloop[i in 2:4], x[i]+y[i]-(x[i-1]+y[i-1])==Kp[i]+Zp[i])     # conservation of boats
@objective(m, Min, 400*sum(x) + 450*sum(y) + 20*sum(h)+400*sum(Kp)+500*sum(Zp))         # minimize costs
optimize!(m)
